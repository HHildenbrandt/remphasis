...
extern "C" {
  ...
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
    return t_speciation + detail::trunc_exp(0, tree[n - 1].brts - t_speciation, pars[0], reng_);
  }


  EMP_EXTERN(double) emp_speciation_rate(void*, double t, const double* pars, unsigned n, const emp_node_t* etree)
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
  ...
  EMP_EXTERN(double) emp_speciation_rate_sum(void*, double t, const double* pars, unsigned n, const emp_node_t* etree)
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
    auto it = std::lower_bound(tree, tree + n, t, detail::node_less{});
    const double N = it->n;
  ...
  EMP_EXTERN(double) emp_intensity(void*, const double* pars, unsigned n, const emp_node_t* etree)
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
    double sum_sigma = 0;
  ...
  EMP_EXTERN(double) emp_loglik(void*, const double* pars, unsigned n, const emp_node_t* etree)
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
    double sum_inte = 0;
  ...
  EMP_EXTERN(double) emp_sampling_prob(void*, const double* pars, unsigned n, const emp_node_t* etree)
  {
    auto tree = reinterpret_cast<const node_t*>(etree);
    double logg = -emp_intensity(nullptr, pars, n, etree);
    tree_t mtree;
    double nb = 0.0;
    double no = tree[0].n;
    double ne = 0.0;
